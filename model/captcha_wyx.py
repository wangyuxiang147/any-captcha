...
class Captcha(object):
    def __init__(self, text, bg, char_custom_fns=None, bg_custom_fn=None, specific_chars=None, **kwargs):
        ...
        # 噪点的可选的字体颜色集合
        self._dot_colors = self._kwargs.get("dot_colors", ["0xffffff"])

        # 验证码干扰线的个数
        self._curve = self._kwargs.get("curve", 0)
        # 验证码干扰线的可选的字体颜色集合
        self._curve_colors = self._kwargs.get("curve_colors", ["0xffffff"])
    ...
    def select_dot_color(self):
        color = CaptchaUtils.random_choice_from(self._dot_colors)

        return color

    # def draw_noise_dot(self, color="0x484848", width=3, number=30):
    def draw_noise_dot(self, color="0x484848", width=1, number=30):
        while number:
            CaptchaUtils.draw_dot(self._captcha, color=CaptchaUtils.get_rgb(self.select_dot_color()), width=width)
            number -= 1
    ...
    def joint_image(self):
        ...
        average = int((self._width-20) / self._num)
        except_x = 10 # 从第10个像素开始
    ...
    def select_curve_color(self):
        curve_color = CaptchaUtils.random_choice_from(self._curve_colors)

        return curve_color

    def create_captcha(self):
        if self._curve:  # 先画干扰线
            self.draw_noise_curve(number=self._curve, color=self.select_curve_color())
        ...
        act_text = []
        for index, i in enumerate(range(self._num)):
            if index >= self._num/2:
                color = self._char_colors[1]
            else:
                color = self._char_colors[0]
            draw = Draw(self._captcha)
            char = specific[i].get("char", self._text[i])
            color = specific[i].get("color", color)
            font = specific[i].get("font", self._char_fonts[1] if (i+1) % 2 == 0 else self._char_fonts[0])
            size = specific[i].get("size", self.select_size())
            rotate = specific[i].get("rotate", 0 if (i+1) % 2 == 0 else self._char_rotate)
